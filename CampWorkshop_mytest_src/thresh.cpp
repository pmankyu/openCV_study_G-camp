...
void adaptive_threshold_test(char *name)
{
	...
	imshow("org", img);
	int C = 0;
	adaptiveThreshold(img, dst, 255, ADAPTIVE_THRESH_MEAN_C, THRESH_BINARY, 19, C);
	imshow("MEAN_C 19", dst);
	adaptiveThreshold(img, dst, 255, ADAPTIVE_THRESH_GAUSSIAN_C, THRESH_BINARY, 19, C);
	imshow("GAUSSIAN 19", dst);
	adaptiveThreshold(img, dst, 255, ADAPTIVE_THRESH_MEAN_C, THRESH_BINARY, 49, C);
	imshow("MEAN_C 49", dst);
	adaptiveThreshold(img, dst, 255, ADAPTIVE_THRESH_GAUSSIAN_C, THRESH_BINARY, 49, C);
	imshow("GAUSSIAN 49", dst);
	adaptiveThreshold(img, dst, 255, ADAPTIVE_THRESH_MEAN_C, THRESH_BINARY, 199, C);
...
void thresh_otsu(char *name) {
	...
	//GaussianBlur(img, blurimg, Size(5, 5),1.0,1.0);

	imshow("blurred", blurimg);
	show_hist(blurimg, "blurred hist");
